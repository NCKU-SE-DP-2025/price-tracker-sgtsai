...
export default {
  ...
  setup(props) {
    ...
    const generateLabels = (startDate, endDate, count) => {
      ...
      chartInstance = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            ...
            tension: 0.1,
            pointRadius: prices.map((_, i) => annotations.some(a => a.index === i) ? 5 : 3),
            pointStyle: prices.map((_, i) => annotations.some(a => a.index === i) ? 'crossRot' : 'circle')
          }]
        },
        options: {
          scales: {
