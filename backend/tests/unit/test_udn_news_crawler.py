...
from backend.app.crawler.udn_crawler import UDNCrawler
from backend.app.crawler.crawler_base import NewsWithSummary
...
class TestUDNCrawler(unittest.TestCase):
    ...
    @patch("backend.app.crawler.udn_crawler.requests.get")
    ...
    def test_parse_news(self, mock_get):
        ...
        self.assertIn("Content paragraph 1.", news.content)
    ...
    def test_is_valid_url(self):
        self.assertTrue(self.scraper._is_valid_url("https://udn.com/news/test-news"))
        self.assertFalse(self.scraper._is_valid_url("https://example.com/news/test-news"))

    def test_parse_invalid_domain(self):
        with self.assertRaises(DomainMismatchException):
            self.scraper.validate_and_parse("https://example.com/news/test-news")

    @patch("backend.app.crawler.udn_crawler.Session")
